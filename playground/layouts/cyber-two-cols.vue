...
<template>
  <div class="slidev-layout bg-cyberpunk h-full p-12 flex flex-col justify-between">
    ...
    <!-- メインコンテンツ（左右2カラム） -->
    <div class="grid grid-cols-2 gap-8 items-start my-auto">
      <div class="space-y-4 text-coolgray-200">
        <slot name="left" />
      </div>
      <div class="space-y-4 text-coolgray-200">
        <slot name="right" />
    ...
    <div class="text-xs text-coolgray-600 font-mono mt-4 flex justify-between border-t border-coolgray-900 pt-2">
      <!-- 💡 footerText が空文字（""）の時は「::」自体を非表示にし、文字があれば表示する -->
      <span v-if="footerText">:: {{ footerText }}</span>
      <span v-else></span> <!-- 空白を維持してページ番号の位置がズレないようにする -->
